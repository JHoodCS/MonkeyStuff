...
%   fixed. External control inputs are stubbed for future integration.
    ...
    % maximized, and register keyboard and close callbacks
    fig = figure( ...
    ...
    % Pause/Play toggle button positioned in the top-right corner
    pauseButton = uicontrol(fig, ...
        'Style', 'togglebutton', ...
        'String', 'Pause', ...
        'Units', 'normalized', ...
        'Position', [0.93 0.955 0.055 0.03], ...
        'FontSize', 10, ...
        'FontWeight', 'bold', ...
    ...
    % Left panel: slide-dependent plots/text (~22.5% of figure width)
    panelLeft = uipanel(fig, ...
        ...
        'ShadowColor', theme.panelEdgeColor);

    % Center panel: persistent 3D orbit scene (~50% of figure width)
    panelCenter = uipanel(fig, ...
        'Units', 'normalized', ...
        'Position', [0.248 0.05 0.504 0.90], ...
        'BackgroundColor', theme.figureColor, ...
        'BorderType', 'none');

    % Right panel: slide-dependent plots/text (~22.5% of figure width)
    panelRight = uipanel(fig, ...
        'Units', 'normalized', ...
        'Position', [0.76 0.05 0.225 0.90], ...
        'BackgroundColor', theme.panelColor, ...
        'BorderType', 'line', ...
        'HighlightColor', theme.panelEdgeColor, ...
        'ShadowColor', theme.panelEdgeColor);
    ...
    function center = initializeOrbitPanel(parentPanel, orbit, sun, themeValue)
        ...
        % Live telemetry text overlay in the lower-left corner of the 3D scene
        center.infoText = text(ax, ...
            orbit.axisLimits(1) + 0.05 * orbit.sceneRadius, ...
            orbit.axisLimits(4) - 0.08 * orbit.sceneRadius, ...
            orbit.axisLimits(6) - 0.10 * orbit.sceneRadius, ...
            '', ...
            'Color', [0.91 0.94 1.00], ...
            'BackgroundColor', [0.05 0.07 0.13], ...
            'Margin', 8, ...
            'FontSize', 10, ...
            'FontWeight', 'bold', ...
            'VerticalAlignment', 'top');
    ...
    function renderFrame()
        ...
        % Update the figure-level title with the current slide name
        sgtitle(fig, slideSpec.title, ...
    ...
    function updateOrbitPanel(center, orbit, sun, frameIdx, slideSpec, controlState)
        ...
        % Refresh the telemetry text overlay (altitude, elevation, band)
        set(center.infoText, 'String', composeCenterInfo(orbit, frameIdx, controlState));
    end
...
function controlConfig = loadControlStub(data, numSteps, big_mat, opMode)
    ...
    % 'AUTO' means the viewer infers the active band from contact flags
    controlConfig.stubCommSelection = repmat({'AUTO'}, 1, numSteps);
...
function slides = buildSlides(big_mat, orbit, opMode)
    ...
    slides(3).rightItems = [ ...
        lossItem, ...
        makePlaceholderItem('Communications Expansion Stub', ...
            sprintf(['TODO: add more comm variables here.\n' ...
                     'Reserved for plots/text such as:\n' ...
                     '- received power\n' ...
                     '- C/N\n' ...
                     '- modem state / lock status']), 0.40)];
...
function bodyText = buildTextBody(textRole, frameIdx, controlState, orbit)
    switch textRole
        ...
        case 'wirelessComm'
            ...
            % C/N and received power are shown as 0 when not in contact and
